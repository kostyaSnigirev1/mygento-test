...
const requiredString = yup
  .string()
  .trim()
  .matches(unicodeReLang, (fildValues) => {
    return `В ${fild[fildValues.path]} могут быть только буквы`;
  })
  .required((fildValues) => {
    return `Пожалуйста укажите ${fild[fildValues?.path]}`;
  });
...
const validationSchema = yup.object().shape({
  ...
  file: yup
    ...
    .test('file', 'File too large', (value) => {
      // console.log(value?.size <= FILE_SIZE);
      console.log(value);
      ...
      <Grid container spacing={4}>
        ...
        <Grid item md={3} xs={12} container alignItems="flex-end">
          <FileInput
            ...
            errorMsg={formik.errors.file && formik.errors.file}
          />
        </Grid>
        <Grid item xs={12}>
